...
const useSearch = (getData: GetDataFunction): UseSearchReturn => {
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    setIsItemSelected(false); // Reset when user types again
  };

  const handleSelect = (item: string): void => {
    setSearchedItem(item);
    setSuggestedItems([]);
    setIsItemSelected(true); // Mark that an item was selected
  };
